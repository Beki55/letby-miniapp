...
const sessionKey = (telegramUserId: number) => `bot:negotiation:${telegramUserId}`;

export const registerNegotiationHandler = (bot: Telegraf<Context>): void => {
  bot.command("negotiate", async (ctx) => {
    ...
    await redis.set(
      sessionKey(userId),
      JSON.stringify({ listingId, step: "awaiting_offer" }),
      "EX",
      60 * 60
    );
  ...
  bot.on("text", async (ctx, next) => {
    ...
    const session = JSON.parse(raw) as { listingId: string; step: string };
    ...
    await redis.set(
      sessionKey(userId),
      JSON.stringify({ ...session, step: "offer_sent", offer }),
      "EX",
      60 * 60
    );

    await ctx.reply(
      `Offer ${offer} ETB recorded for listing ${session.listingId}. Seller will be notified.`
    );
  });
};
